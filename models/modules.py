...
class ApproachNet(nn.Module):
    ...
    def forward(self, seed_xyz, seed_features, end_points):
        ...
        view_score = features[:, 2:2 + self.num_view, :].transpose(
            1, 2).contiguous()  # (B, num_seed, num_view)
        ...
        template_views = generate_grasp_views(self.num_view).to(
            features.device)  # (num_view, 3)
        template_views = template_views.view(1, 1, self.num_view, 3).expand(
            B, num_seed, -1, -1).contiguous()  #(B, num_seed, num_view, 3)
...
class CloudCrop(nn.Module):
    """Cylinder group and align for grasp configure estimation. Return a list
    of grouped points with different cropping depths.
    ...
    def forward(self, seed_xyz, pointcloud, vp_rot):
        ...
        for grouper in self.groupers:
            grouped_features.append(grouper(
                pointcloud, seed_xyz,
                vp_rot))  # (batch_size, feature_dim, num_seed, nsample)
        ...
        vp_features = F.max_pool2d(
            vp_features,
            kernel_size=[1, vp_features.size(3)
...
class OperationNet(nn.Module):
    ...
    def forward(self, vp_features, end_points):
        ...
        end_points['grasp_angle_cls_pred'] = vp_features[:, self.num_angle:2 *
                                                         self.num_angle]
        end_points['grasp_width_pred'] = vp_features[:, 2 * self.num_angle:3 *
                                                     self.num_angle]
        return end_points
