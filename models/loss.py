...
from loss_utils import (GRASP_MAX_TOLERANCE, GRASP_MAX_WIDTH, THRESH_BAD,
...
def compute_objectness_loss(end_points):
    ...
    end_points['stage1_objectness_prec'] = (
        objectness_pred == objectness_label.long()
...
def compute_grasp_loss(end_points, use_template_in_training=True):
    ...
    target_angles = torch.gather(top_view_grasp_angles, 2,
                                 target_labels_inds).squeeze(2)  # (B, Ns, D)
    target_depths = torch.gather(top_view_grasp_depths, 2,
                                 target_labels_inds).squeeze(2)  # (B, Ns, D)
    target_widths = torch.gather(top_view_grasp_widths, 2,
                                 target_labels_inds).squeeze(2)  # (B, Ns, D)
    target_tolerance = torch.gather(
        batch_grasp_tolerance, 2, target_labels_inds).squeeze(2)  # (B, Ns, D)
    ...
    acc_mask_15 = (
        (torch.abs(grasp_angle_class_pred - target_angles_cls) <= 1) |
        (torch.abs(grasp_angle_class_pred - target_angles_cls) >= A - 1))
    end_points['stage2_grasp_angle_class_acc/15_degree'] = acc_mask_15[
        loss_mask.bool()].float().mean()
    acc_mask_30 = (
        (torch.abs(grasp_angle_class_pred - target_angles_cls) <= 2) |
        (torch.abs(grasp_angle_class_pred - target_angles_cls) >= A - 2))
    end_points['stage2_grasp_angle_class_acc/30_degree'] = acc_mask_30[
        loss_mask.bool()].float().mean()

    # 3. width reg loss
    grasp_width_pred = torch.gather(end_points['grasp_width_pred'], 1,
                                    target_labels_inds_).squeeze(1)
    ...
    grasp_tolerance_pred = torch.gather(end_points['grasp_tolerance_pred'], 1,
                                        target_labels_inds_).squeeze(1)
