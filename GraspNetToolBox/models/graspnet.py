...
from utils.label_generation import batch_viewpoint_params_to_matrix, match_grasp_view_and_label, process_grasp_labels
...
class GraspNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.grasp_generator = GraspNetStage2(num_angle, num_depth,
...
def pred_decode(end_points):
    ...
    for i in range(batch_size):
        ...
        grasp_score = torch.gather(grasp_score, 0,
                                   grasp_angle_class_).squeeze(0)
        grasp_width = torch.gather(grasp_width, 0,
                                   grasp_angle_class_).squeeze(0)
        grasp_tolerance = torch.gather(grasp_tolerance, 0,
                                       grasp_angle_class_).squeeze(0)
        ...
        grasp_preds.append(
            torch.cat([
                ...
                rotation_matrix, grasp_center, obj_ids
            ],
                      axis=-1))
    return grasp_preds
